...
(def-test-method test-cache-memory ((test cache-test))
  (let ((cache (make-instance 'cache :item-type :int :total-items 10)))
    ...
    (assert-true (null-pointer-p (start-address cache))
                 "start-address not de-allocated")))